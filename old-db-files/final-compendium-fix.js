...
// Create folder structure - Using proper Foundry Folder format
const folders = {};
const folderOrder = [
  'La Familia',
  'Criminal Conspiracy', 
...
// Create folder entries - Folders in compendiums are different from world folders
folderOrder.forEach((folderName, index) => {
  const folderId = generateFoundryId();
  folders[folderName] = folderId;
});

// Convert to Foundry JSON format - items only, no separate folder documents
const foundryItems = data.map((item, index) => {
  ...
  return {
    ...
    effects: [],
    folder: folderId,
    sort: (folderOrder.indexOf(folder) * 10000) + (index * 100),
    ownership: {
      default: 0
    },
    flags: {},
...
// Write as .db file (items only, folders will be created automatically by Foundry)
...
console.log(`Created ${foundryItems.length} group abilities with folder references`);
console.log(`Total entries written to: ${outputPath}`);

// Log the groups found
console.log('\nItems created by folder:');
folderOrder.forEach(folder => {
  const itemCount = foundryItems.filter(item => item.system.folder === folder).length;
  const folderId = folders[folder];
  console.log(`- ${folder} (${folderId}): ${itemCount} abilities`);
});
...
foundryItems.slice(0, 3).forEach(item => {
  console.log(`${item.name}: "${item.system.effect}" (folder: ${item.folder})`);
});
